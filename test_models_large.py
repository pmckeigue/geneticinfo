"""Compare models on larger simulated case-control data with more related pairs."""
import jax
...
# --- Default simulation: 100k fullsib, 10k halfsib, 100k unrelated ---
...
# ====================================================================
print("\n" + "=" * 60)
print(f"1. logistic_mvnorm with NUTS  (M={M})")
print("=" * 60)

kernel = NUTS(logistic_mvnorm, max_tree_depth=8)
...
beta0_nuts = samples_nuts["beta0"]

print(f"\n  True: mu={info['mu']:.4f}, s={info['s']:.4f}")
print(f"  s:     mean={float(s_nuts.mean()):.4f}, median={float(jnp.median(s_nuts)):.4f}, sd={float(s_nuts.std()):.4f}")
print(f"  mu:    mean={float(mu_nuts.mean()):.4f}, median={float(jnp.median(mu_nuts)):.4f}, sd={float(mu_nuts.std()):.4f}")
print(f"  beta0: mean={float(beta0_nuts.mean()):.4f}, median={float(jnp.median(beta0_nuts)):.4f}")

# Print only global parameters
import io, sys
buf = io.StringIO()
old = sys.stdout
sys.stdout = buf
...
for line in buf.getvalue().splitlines():
    if any(k in line for k in ["beta0", "mu ", " s ", "n_eff", "r_hat", "diverge"]):
        print("  " + line)
...
# ====================================================================
print("\n" + "=" * 60)
print(f"2. logistic_mix2_mvnorm with SVI  (M={M})")
print("=" * 60)
...
svi_result = svi.run(random.key(42), 15000, M=M, L=L, y=y, progress_bar=True)
...
print(f"\n  True: mu={info['mu']:.4f}, s={info['s']:.4f}")
print(f"  s:     mean={float(s_svi.mean()):.4f}, median={float(jnp.median(s_svi)):.4f}, sd={float(s_svi.std()):.4f}")
print(f"  mu:    mean={float(mu_svi.mean()):.4f}, median={float(jnp.median(mu_svi)):.4f}, sd={float(mu_svi.std()):.4f}")
